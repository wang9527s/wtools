    ...
    ui(new Ui::TimerTask)
{
    ...
    connect(ui->tasktypeCbx,&QComboBox::currentTextChanged,this,&TimerTask::on_taskType_textChanged);
...
bool TimerTask::getInputTask(TimerTaskInfo &task)
{
    ...
    task.type = ui->tasktypeCbx->currentData().toInt(); //1 每周任务        2 每天任务      3 一次性定时任务
