...
void RSSUpdateOPML::updateBilibili(const QString & rssServer,const QString & vmid,const QString & opmlPath)
{
    // opml文件中已经存在该id
...
QStringList RSSUpdateOPML::getFollowings(const QString & rssServer,const QString &vmid)
{
    ...
    QString cmd=QString("go run ~/wwork/1.bin/rss-bilibli.go %1 > %2")
