...
public:
    ...
    static OSStatus
    hotkeyPressEventHandler(EventHandlerCallRef nextHandler, EventRef event, void *data);
    static OSStatus
    hotkeyReleaseEventHandler(EventHandlerCallRef nextHandler, EventRef event, void *data);
...
bool QHotkeyPrivateMac::registerShortcut(QHotkey::NativeShortcut shortcut)
{
    if (!this->isHotkeyHandlerRegistered) {
        ...
        InstallApplicationEventHandler(
            &QHotkeyPrivateMac::hotkeyPressEventHandler, 1, &pressEventSpec, NULL, NULL);
        ...
        InstallApplicationEventHandler(
            &QHotkeyPrivateMac::hotkeyReleaseEventHandler, 1, &releaseEventSpec, NULL, NULL);
    }
    ...
    OSStatus status = RegisterEventHotKey(
        shortcut.key, shortcut.modifier, hkeyID, GetApplicationEventTarget(), 0, &eventRef);
    if (status != noErr) {
...
OSStatus QHotkeyPrivateMac::hotkeyPressEventHandler(EventHandlerCallRef nextHandler,
                                                    EventRef event,
                                                    void *data)
{
...
OSStatus QHotkeyPrivateMac::hotkeyReleaseEventHandler(EventHandlerCallRef nextHandler,
                                                      EventRef event,
                                                      void *data)
{
    Q_UNUSED(nextHandler);
